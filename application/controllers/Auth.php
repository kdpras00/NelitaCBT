...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->form_validation->run() != false) {
			...
			$kepsek 	= $this->db->get_where('tb_kepsek', ['username' => $username])->row_array();
			...
			if ($admin && $password == $admin['password']) {
				...
				// $this->session->set_flashdata('message', '<div class="alert alert-success alert-message text-center"><b>Login Berhasil !,<br></b> Halaman ini akan dialihkan ke Halaman Home</div>');
				redirect(base_url('home'));
			} else if ($kepsek && $password == $kepsek['password']) {
				$data = $this->m_data->edit_data($where, 'tb_kepsek')->row();
				$data_session = array(
					'id'		=> $data->id,
					'nama'		=> $data->nama_kepsek,
					'username'	=> $data->username,
					'status'	=> 'kepsek_login'
				);
				$this->session->set_userdata($data_session);
